...
<tr class="product_row">
    ...
    <td>
        ...
        <input type="text" class="form-control quantity quantity_{{$i}}" data-val="0"  name="add_stock_lines[{{$i}}][quantity]" required min="{{ isset($product->units->pluck('name')[0]) &&  in_array(strtolower($product->units->pluck('name')[0]), ['piece', 'قطعة', 'قطعه', ''], true) ? 1 : 0.00001 }}" value="0"/>
...
{{--        <input class="final_cost" type="hidden" name="add_stock_lines[{{$i}}][final_cost]" value="@if(isset($product->default_purchase_price)){{@num_format($product->default_purchase_price / $exchange_rate)}}@else{{0}}@endif">--}}
    ...
    <td>
        <span class="sub_total_span sub_total_span{{$product->id}}"></span>
