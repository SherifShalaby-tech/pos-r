...
<tr>
    ...
    <td>@if($product->is_service){{'-'}}@else{{preg_match('/\.\d*[1-9]+/', (string)$product->current_stock) ?
        $product->current_stock : @num_format($product->current_stock)}}@endif</td>
    ...
    <td class="qty_hide @if ($type != 'package_promotion') hide @endif">
        <input type="text" class="qty product_qty_checkbox form-control"
            name="package_promotion_qty[{{$product->variations_id}}]" id="" value="{{$q}}">

        <input type="hidden" class="product_variations_ids form-control"
            name="product_variation_id[{{$product->variations_id}}]" id="" value="{{$product->variations_id}}">

    </td>
    <td><button type="button" class="btn  btn-danger remove-rounded remove_row_sp"
