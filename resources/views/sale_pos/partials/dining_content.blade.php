            ...
            <div class="vertical-tab" role="tabpane">
                ...
                <ul class="nav nav-tabs" role="tablist">
                    ...
                    <li role="presentation"
                        ...
                        <a class="@if ($loop->index == 0 && empty($active_tab_id)) active show @elseif($dining_room->id == $active_tab_id) active @endif room{{$dining_room->id}}"
                            ...
                            data-toggle="tab"><span
                                class="badge badge-danger room-count-badge room-badge{{$dining_room->id}} hide ">0</span>{{
                            $dining_room->name }}</a>
                    </li>
                    ...
                    <li role="presentation"><a data-href="{{ action('DiningRoomController@create') }}"
                            style=" color: #fff; font-size:12px; padding: 18px 10px 16px; display:block "
                            class="bg-primary" data-container=".view_modal" class="btn btn-modal add_dining_room"
                            aria-controls="messages" role="tab">@lang('lang.add_dining_room')</a></li>
                </ul>
                <!-- Tab panes -->
                <div class=" tab-content tabs tables">
                    @foreach ($dining_rooms as $dining_room)
                    <div role="tabpane"
                        ...
                        <div class="row" style="line-height: 13px;">
                            ...
                            <div class="col-md-2 text-center">
                                {{-- <button type="button" class="btn btn-sm text-danger remove-table float-left"
                                    style="border-radius: 70%;" data-table_id="{{ $reserve->dining_table_id }}"><i
                                        class="fa fa-times"></i></button> --}}
                                <div class="table_action" data-table_id="{{ $reserve->dining_table_id }}">
                                    <span
                                        class="badge badge-danger selected-table-badge table{{$reserve->dining_table_id}} hide">0</span>
                            ...
                            <div class="col-md-2 text-center">
                                ...
                                <span
                                    class="badge badge-danger selected-table-badge table{{$reserve->dining_table_id}} hide">0</span>
                                @php
                                $reserveHasOrder = App\Models\TableReservation::where('dining_table_id',
                                ...
                                <a href="{{ action('SellPosController@edit', $reserveHasOrder->current_transaction_id) }}"
                                    data-transaction_id="{{$reserveHasOrder->current_transaction_id}}" target="_blank"
                                    rel="noopener noreferrer" class="table-link"
                                    data-room="{{$dining_table->dining_room_id}}"
                                    data-table_no="{{$reserve->dining_table_id}}">
                                    @endif --}}
                                    <div class="text-center">
                                        ...
                                        @endif
                                        <p style="padding: 0px; margin: 0px; color:black;">
                                            <button type="button" class="btn btn-sm text-danger table_action_reserve"
                                                data-table_id="{{ $reserve->dining_table_id}}"
                                                style="border-radius: 70%;"><i
                                                    class="fa fa-plus"></i>&nbsp;@lang('lang.new_reserve')</button>
                                        </p>
                                        @if(!empty($reserveHasOrder->current_transaction_id))
                                        <a href="{{ action('SellPosController@edit', $reserveHasOrder->current_transaction_id) }}"
                                            data-transaction_id="{{$reserveHasOrder->current_transaction_id}}"
                                            ...
                                            <p style="padding-top: 4px; margin: 0px; color:black;">
                                                <button type="button" class="btn btn-sm btn-danger text-white"
                                                    data-table_id="{{ $reserve->dining_table_id }}"
                                                    data-table_id="{{ $reserve->dining_table_id}}"
                                                    style="border-radius: 70%;">@lang('lang.order')</button>
                                            </p>
                                        </a>
                                        @else
                                        <p style="padding-top: 4px; margin: 0px; color:black;">
                                            <button type="button" class="btn btn-sm text-danger table_action"
                                                data-table_id="{{ $reserve->dining_table_id }}"
                                                data-table_id="{{ $reserve->dining_table_id}}"
                                                style="border-radius: 70%;"><i
                                    ...
                                    @php
                                    $reservations = App\Models\TableReservation::where('dining_table_id',
                                    $reserve->dining_table_id)
                                    ...
                                    <div class="dropdown reservations-dropdown">
                                        ...
                                        <ul class="dropdown-menu">
                                            ...
                                            <li class="d-flex align-items-center justify-content-between">
                                                <span style="cursor:pointer;" class="table_edit_reserve_btn"
                                                    data-table_id="{{ $table->dining_table_id }}"
                                                    data-reserve_id="{{ $table->id }}">*{{ $table->customer_name
                                                    }}</span>

                                                <button type="button" class="btn btn-danger btn-sm cancel_reserve"
                                    ...
                                    <div class="text-center pt-2">
                                        <p data-table_id="{{ $reserve->dining_table_id }}"
                                            style="padding: 0px; margin: 0px; color:red;cursor:pointer;"
                                            class="table_cancel_reserve_btn">
                            ...
                            <div class="col-md-2 text-center">
                                {{-- <button type="button" class="btn text-danger btn-sm remove-table float-left"
                                    style="border-radius: 70%;" data-table_id="{{ $reserve->dining_table_id }}"><i
                                        class="fa fa-times"></i></button> --}}
                                <div class="order_table" data-table_id="{{ $reserve->id }}">
                                    <span
                                        class="badge badge-danger selected-table-badge table{{$reserve->dining_table_id}} hide">0</span>
                                    <a href="{{ action('SellPosController@edit', $reserve->current_transaction_id) }}"
                                        data-transaction_id="{{$reserve->current_transaction_id}}" target="_blank"
                                        rel="noopener noreferrer" class="table-link"
                                        data-room="{{$dining_table->dining_room_id}}"
                                        data-table_no="{{$reserve->dining_table_id}}">
                                        <p style="padding: 0px; margin: 0px; color:red;">
                            ...
                            <div class="col-md-2">
                                <button class="btn btn-modal add_dining_table bg-primary"
                                    style=" padding: 18px 10px 16px; color: #fff; margin-top: 15px;"
